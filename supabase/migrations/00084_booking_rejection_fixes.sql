-- 00084 — fix two participant-booking rejection bugs.
--
-- Bug #1 (DUPLICATE_PARTICIPATION over-matches): book_slot's duplicate gate
-- (00069:89-98) used `p.phone = $phone OR p.email = $email`. Two DIFFERENT
-- legitimate people who share EITHER a phone OR an email (a couple listing one
-- guardian phone, a shared household email, a mistyped field) blocked each
-- other with DUPLICATE_PARTICIPATION. The participant identity key is the
-- (phone, email) UNIQUE pair used by the upsert 8 lines later — so the gate
-- must match the SAME way: AND, not OR.
--
-- Bug #2 (recruitment auto-close never reopens): when recruitment_target or
-- auto_lock is reached, book_slot flips experiments.status='completed'. If a
-- recruited participant later CANCELS, a slot frees but the experiment stays
-- 'completed', so the next legitimate participant gets EXPERIMENT_NOT_FOUND.
-- Fix: mark auto-closes with experiments.recruitment_auto_closed, and add
-- reopen_experiment_if_undersubscribed() that the cancel path calls to flip an
-- AUTO-closed, now-undersubscribed, not-yet-ended experiment back to 'active'.
-- The marker guarantees we NEVER reopen a manually-completed experiment.
--
-- Idempotent: ADD COLUMN IF NOT EXISTS + CREATE OR REPLACE FUNCTION. book_slot
-- keeps the same signature so its existing GRANTs persist.

ALTER TABLE experiments
  ADD COLUMN IF NOT EXISTS recruitment_auto_closed boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN experiments.recruitment_auto_closed IS
  'true면 book_slot이 정원(recruitment_target/auto_lock) 도달로 자동 완료시킨 실험. 취소로 결원이 생기면 reopen_experiment_if_undersubscribed가 active로 되돌린다. 연구원이 수동 완료한 실험은 false로 남아 자동 재오픈 대상이 아니다.';
...
BEGIN
  ...
  -- Bug #1 fix: identity is the (phone, email) pair (the upsert unique key),
  -- so the duplicate gate matches on BOTH fields, not EITHER. Two different
  -- people sharing one field no longer block each other.
  SELECT COUNT(*) INTO v_existing_count
  FROM bookings b
  JOIN participants p ON p.id = b.participant_id
  WHERE b.experiment_id = p_experiment_id
    AND b.status = 'confirmed'
    AND p.phone = p_participant_phone
  ...
  IF v_experiment.recruitment_target IS NOT NULL THEN
    ...
    IF v_recruited_count >= v_experiment.recruitment_target THEN
      UPDATE experiments SET status = 'completed', recruitment_auto_closed = true
...
-- Bug #2 fix: reopen an AUTO-closed experiment when a cancellation drops it
-- below its own auto-close threshold. Only touches experiments the system
-- itself closed (recruitment_auto_closed = true) and only while still within
-- end_date — a manually-completed or ended experiment is never reopened.
CREATE OR REPLACE FUNCTION reopen_experiment_if_undersubscribed(
  p_experiment_id uuid
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('reopened', false, 'reason', 'NOT_FOUND');
  END IF;
  -- Only reopen what the SYSTEM auto-closed; never a manual completion.
  IF v_experiment.status <> 'completed' OR NOT v_experiment.recruitment_auto_closed THEN
    RETURN jsonb_build_object('reopened', false, 'reason', 'NOT_AUTO_CLOSED');
  END IF;
  -- Don't resurrect a run whose recruitment window has ended.
  IF v_experiment.end_date IS NOT NULL
     AND v_experiment.end_date < (now() AT TIME ZONE 'Asia/Seoul')::date THEN
    RETURN jsonb_build_object('reopened', false, 'reason', 'PAST_END_DATE');
  END IF;

  IF v_experiment.recruitment_target IS NOT NULL THEN
    SELECT COUNT(DISTINCT b.participant_id) INTO v_recruited
    FROM bookings b
    WHERE b.experiment_id = p_experiment_id
      AND b.status IN ('confirmed', 'running', 'completed', 'no_show');
    IF v_recruited < v_experiment.recruitment_target THEN
      v_should_reopen := true;
    END IF;
  END IF;

  IF NOT v_should_reopen AND v_experiment.auto_lock THEN
    SELECT COUNT(*) INTO v_total_confirmed FROM bookings
    WHERE experiment_id = p_experiment_id AND status = 'confirmed';
    SELECT experiment_total_capacity(v_experiment) INTO v_total_capacity;
    IF v_total_capacity IS NOT NULL AND v_total_capacity > 0
  ...
  IF v_should_reopen THEN
    UPDATE experiments
      ...
      WHERE id = p_experiment_id AND status = 'completed';
    RETURN jsonb_build_object('reopened', true);
  END IF;

  RETURN jsonb_build_object('reopened', false, 'reason', 'STILL_FULL');
END;
$$;

REVOKE EXECUTE ON FUNCTION reopen_experiment_if_undersubscribed(uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION reopen_experiment_if_undersubscribed(uuid) FROM anon, authenticated;
GRANT  EXECUTE ON FUNCTION reopen_experiment_if_undersubscribed(uuid) TO service_role;
