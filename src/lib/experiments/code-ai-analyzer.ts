...
const GENERAL_RULES = [
  ...
  "1. **factors (조작 변수, IV)**: 실험자가 *의도적으로 변형*해 효과를 측정하는 변수. 코드에서 피험자/세션/날짜/조건/블럭 마다 *다른 값*을 갖는 것.",
  "   role 분류: between_subject / within_subject / within_session / per_trial / derived / unknown.",
  "2. **parameters (셋업 상수)**: 모든 trial에서 *고정*된 셋업 값. timing, screen geometry, stimulus 셋업, 파일 경로 등.",
  "   shape 분류: constant / vector / expression / input / unknown.",
  ...
  "6. **saved_variables**: 데이터 sink (파일 저장 / data 객체 / addData / to_csv / np.save / save / writeFile / 인덱스 ` (iR)(iT)`) 모두. per-trial / per-block / per-session 모두.",
  "7. **meta.block_phases**: 한 세션에 phase 가 여럿이면 (예: training+test, practice+main, stair+train+test, Day1=training/Day2~5=test) 분리해 배열로. 단일 n_blocks 로 평탄화하지 말 것.",
  "8. **meta.design_matrix**: 피험자/세션 별 IV 배정 패턴 (예: `subjNum mod 4 → AABB/ABBA/...`) 은 자연어 설명으로 여기 넣고, conditions 에 cartesian 으로 풀지 말 것.",
  "9. **meta.domain_genre**: 코드의 task 구조와 saved_variable 모양으로 추정 (psychophysics / estimation / decision / retrieval / search / memory / motor / categorization / attention / imagery / language / social / gamified / perception / other).",
  "10. **헤더 주석은 changelog**: `% Timing: tprecue 0.5->0.3` 같은 주석은 *변경 이력*. 본문 할당이 있으면 본문 우선.",
  "11. **확신 없으면 null + warnings 한국어 1줄 사유**.",
  "12. **line_hint**: 1-based 라인. 모르면 null.",
];
...
const FRAMEWORK_AUGS: Record<string, string[]> = {
  ...
  psychtoolbox: [
    ...
    "- `par.condition = N * ones(1,nBlocks)` 처럼 단일값 곱셈은 *상수* — IV 가 아님.",
    "- `par.nT = [stair train test]` 배열에서 nonzero 원소가 활성 stage. 0 인 stage 는 미실행.",
    "- `par.StairTrainTest = [1 1 2 2 3 3]` 처럼 cell 배열은 within-session block-kind factor.",
    "- `if par.day == 1 nBlocks = N1; else nBlocks = N2;` → meta.block_phases 두 항목 (training day vs test day).",
    "- `par.results.X(iR)(iT) = …`, `par.tp.X{iR}(iT) = …` 패턴은 per-trial saved_variable. cell-of-cell 는 채널별로 풀어 등록.",
    "- `save(filename, 'struct')` 가 session-level 저장 sink.",
    "- PTB GetSecs 결과는 sec 단위.",
  ],
  "lab.js": [
    "**lab.js 패턴 힌트**:",
...
export async function runAiAnalysis(input: AiAnalyzeInput): Promise<AiAnalyzeResult> {
    ...
    [
      docs
        ? "참고 문서(연구자 작성 — 실험 설계의 ground truth로 우선 신뢰):\n```\n" +
          ...
          "\n```\n"
        : "",
      "아래 메타데이터(seed)와 코드를 보고 스키마에 맞는 JSON 만 출력하세요.",
      "```json",
  ...
  // Save-focused output commonly hits 6-8K JSON tokens (50+
  // saved_variables × 6 fields each). 12288 is the sweet spot for
  // qwen3.6 / claude-opus-4-7 alike.
