...
4. **Researcher-initial → calendar title.** ✅ Partial (iter 25, 2026-05-30) — `creatorInitial` + `formatKrPhone` 가 `src/lib/google/title-helpers.ts` 단일 owner. drift (booking.service.ts 에는 slice 없고 gcal-retry 에는 slice(0,4)) 가 runtime semantics (no-slice) 로 통일 — 같은 연구자가 첫 시도 vs retry 경로에서 다른 calendar title 받던 버그 해소. `calendarTitle(...)` 자체와 `calendarDescription(...)` 의 booking-row-shape 의존 부분은 booking.service.ts 에 남음 — 완전 추출은 Phase B B5 (calendar gateway) 에서.
