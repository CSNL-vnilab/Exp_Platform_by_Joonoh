...
4. **Researcher-initial → calendar title.** ✅ (iter 25 + 30, 2026-05-30) — `creatorInitial` + `formatKrPhone` + `calendarTitle` 모두 `src/lib/google/title-helpers.ts` 단일 owner. iter 25 가 creatorInitial drift 해소 + formatKrPhone dedup, iter 30 가 calendarTitle 추출 (booking row shape 를 `{ creator, experimentTitle, projectName, subjectNumber, sessionNumber }` flat 으로 전환). 같은 연구자가 첫 시도 vs retry 경로에서 다른 calendar title 받던 버그 완전 해소. `calendarDescription` 만 보류 — gcal-retry 가 escapeHtml 적용 / runtime 은 안 함, 의도적 drift 의심으로 분리 유지 (참여자 이름에 `<` 가 들어가는 흔치 않은 경우에만 영향).
