...
4. **Researcher-initial → calendar title.** ✅ 완전 (iter 25 + 30 + 32, 2026-05-30) — `creatorInitial` + `formatKrPhone` + `calendarTitle` + `calendarDescription` 모두 `src/lib/google/title-helpers.ts` 단일 owner. iter 25 가 creatorInitial drift 해소 + formatKrPhone dedup, iter 30 가 calendarTitle 추출, iter 32 가 calendarDescription 추출 + escapeHtml drift 해소 (gcal-retry 만 적용하던 escapeHtml 제거 — modern Calendar 클라이언트가 plain text 표시, runtime path 가 escape 없이 production 운영 중이었음). 같은 연구자/참여자가 runtime vs retry 경로에서 다른 calendar event 받던 버그 완전 해소.
